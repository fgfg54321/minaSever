...
public class ProtocolClientBaseDecoder extends CumulativeProtocolDecoder
{
          
      protected HashMap<Long, NetDataPackage>           streamDataDic       = new HashMap<Long, NetDataPackage>();
      protected HashMap<Integer, Class<ClientBaseRequest>>    messageHandler      = new HashMap<Integer, Class<ClientBaseRequest>>();
        	...
        	if(len > in.remaining())
        	{
                ...
                while (iter.hasNext())
                {
                    ...
                    if (nPackage.IsReceiveFull())
                    {
                        ...
                        {
                            ClientBaseRequest netResponse   = messageHandler.get(messageId).newInstance();
                            try
                            {
